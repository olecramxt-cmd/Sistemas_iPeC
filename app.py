...
# Programa app.py. Versão do Código: v.1.5.084
# Data de atualização: 28/07/2026 - 07:21
# Descrição das Alterações:
#   - Implementação da rotina real de importação inteligente de arquivos TXT, com cruzamento de dados, prevenção de duplicidade, atualização de turma/período e marcação real de transferências com data e histórico funcional.
# ==============================================================================
...
st.sidebar.markdown("""
    <div class="sidebar-logo-footer">
        Versão: v.1.5.084 de 28/07/2026<br>
        ...
        if menu_principal == "📊 Painel de Controle de Conformidade e Indicadores de Alunos":
            ...
            if df_db_ano.empty and sub_conformidade == "Atualização de Dados":
                ...
                if sub_conformidade == "Cadastro de Alunos":
                    ...
                    st.success(f"Banco de dados ativo ({ano_letivo_escolhido}) with {total_reg_alunos} registros oficiais na nuvem. | 👦 Masculino: {tot_m} - {pct_m:.1f}% | 👧 Feminino: {tot_f} - {pct_f:.1f}%")
                    
                    st.markdown("#### 🔍 Busca Rápida e Inteligente de Alunos")
                    
                    def atualizar_filtro_aluno():
                        st.session_state.f_aluno = st.session_state.input_f_aluno_v84
                    def atualizar_filtro_mae():
                        st.session_state.f_mae = st.session_state.input_f_mae_v84
                    def atualizar_filtro_turma():
                        st.session_state.f_turma = st.session_state.input_f_turma_v84
                    def atualizar_filtro_turno():
                        st.session_state.f_turno = st.session_state.input_f_turno_v84
                    def atualizar_filtro_status():
                        st.session_state.f_status = st.session_state.input_f_status_v84
                    def atualizar_filtro_pbf():
                        st.session_state.f_pbf = st.session_state.input_f_pbf_v84

                    filtro_cols = st.columns(2)
                    with filtro_cols[0]:
                        st.text_input("Filtrar por Aluno:", value=st.session_state.f_aluno, key="input_f_aluno_v84", on_change=atualizar_filtro_aluno)
                        st.text_input("Filtrar por Mãe:", value=st.session_state.f_mae, key="input_f_mae_v84", on_change=atualizar_filtro_mae)
                        st.text_input("Filtrar por Turma:", value=st.session_state.f_turma, key="input_f_turma_v84", on_change=atualizar_filtro_turma)
                    with filtro_cols[1]:
                        st.text_input("Filtrar por Turno:", value=st.session_state.f_turno, key="input_f_turno_v84", on_change=atualizar_filtro_turno)
                        st.text_input("Filtrar por Status:", value=st.session_state.f_status, key="input_f_status_v84", on_change=atualizar_filtro_status)
                        st.text_input("Filtrar por PBF (Sim/Não):", value=st.session_state.f_pbf, key="input_f_pbf_v84", on_change=atualizar_filtro_pbf)
                    ...
                    if df_filtrado.empty:
                        ...
                        def colorir_transferidos(row):
                            ...
                            if transf_val and transf_val not in ["Não informado", "", "nan", "None"]:
                                return ['color: red; font-weight: bold;'] * len(row)
                            return [''] * len(row)

                        df_estilizado = df_filtrado.style.apply(colorir_transferidos, axis=1)
                        st.dataframe(df_estilizado, use_container_width=True, hide_index=True, key="editor_dados_tabela_v84")
                        ...
                        if st.session_state["perfil_usuario"] == "Total":
                            ...
                            with col_bt3:
                                ...
                                aluno_para_excluir = st.selectbox("Selecionar para Exclusão:", lista_excluir_op, key="sel_exc_painel_v84")
                    ...
                    if df_db_ano.empty:
                        ...
                        aluno_selecionado_busca = st.selectbox("Selecione o aluno para alteração individual:", lista_alunos_cadastrados, key="sel_aluno_atualizacao_individual_v84")
                        ...
                        if aluno_selecionado_busca and aluno_selecionado_busca != "Selecione o Aluno...":
                            try:
                                ...
                                if not df_aluno_ind.empty:
                                    ...
                                    with st.form(f"form_atualizacao_individual_v84_{id_alvo_ind}"):
            ...
            if st.session_state["perfil_usuario"] != "Total":
                ...
                if sub_lote == "Importar Arquivo .TXT":
                    ...
                    arquivos_escolhidos = st.file_uploader("Escolha os arquivos de lote (.TXT)", accept_multiple_files=True, key="upl_lote_txt_v84")
                    
                    if arquivos_escolhidos:
                        if st.button("🚀 Processar Importação Inteligente em Lote", key="btn_proc_lote_v84"):
                            try:
                                ...
                                # Leitura e extração do conteúdo dos arquivos TXT carregados
                                ...
                                if not df_lote_novo.empty and len(registros_nuvem_raw) > 1:
                                    ...
                                    alunos_encontrados_no_lote = set()
                                    ...
                                    for idx_l, reg_linha in enumerate(linhas_dados_nuvem):
                                        if len(reg_linha) > max(idx_aluno, idx_ano):
                                            ...
                                            if ano_reg == str(ano_letivo_escolhido):
                                                ...
                                                mae_nuvem = str(reg_linha[idx_mae]).strip().upper() if len(reg_linha) > idx_mae else ""
                                                
                                                match_lote = df_lote_novo[(df_lote_novo["Aluno"].str.upper() == nome_nuvem)]
                                                if not match_lote.empty:
                                                    alunos_encontrados_no_lote.add(nome_nuvem)
                                                    ...
                                                    # Aluno não está no lote importado -> Marcado como transferido
                                ...
                                registrar_log_auditoria(st.session_state["email_usuario"], st.session_state["perfil_usuario"], f"Executou importação inteligente real para {ano_letivo_escolhido}.")
                                st.success(f"🎉 Importação inteligente executada com sucesso! Atualizados: {atualizados_turma_count} | Transferidos marcados: {transferidos_count}. Verifique o Histórico abaixo.")
                                st.cache_data.clear()
                                st.session_state["dados_banco"] = carregar_banco_dados_virtual()
                                st.rerun()
            ...
            if sub_biblioteca == "Catálogo do Acervo":
                ...
                with col_p1:
                    termo_titulo = st.text_input("Filtrar por Título da Obra:", key="f_tit_v84")
                with col_p2:
                    termo_autor = st.text_input("Filtrar por Autor / Organizador:", key="f_aut_v84")
                with col_p3:
                    filtro_cat = st.selectbox("Filtrar por Categoria:", ["Todas", "Didático", "Literário"], key="f_cat_v84")
                ...
                with st.form("form_biblioteca_v84", clear_on_submit=False):
                ...
                if st.session_state.get("acionou_exclusao_form", False):
                    ...
                    confirma_excluir_form = st.radio("Deseja realmente prosseguir com a exclusão deste livro?", ["Não", "Sim"], index=0, key="radio_conf_exc_v84")
                ...
                with st.form("form_config_biblioteca_v84"):
                ...
                sub_aba_emp = st.radio("Gestão de Circulação:", ["Novo Empréstimo", "Consulta de Empréstimos por Aluno", "Empréstimos Ativos / Devoluções / Atrasos", "Reservas de Livros"], horizontal=True, key="sub_aba_emp_v84")
                
                if sub_aba_emp == "Novo Empréstimo":
                    aluno_emp_sel = st.selectbox("Selecione o Leitor (Aluno):", ["Selecione..."] + lista_alunos_op_global, key="sel_leitor_v84")
                    livro_emp_sel = st.selectbox("Selecione o Item do Acervo (Livro):", ["Selecione..."] + lista_livros_op_global, key="sel_livro_v84")
                    data_emp = st.date_input("Data do Empréstimo:", value=hoje_dt, key="dt_emp_v84", format="DD/MM/YYYY")
                    ...
                    data_prev = st.date_input("Devolver até:", value=data_prev_calc, key="dt_prev_v84", format="DD/MM/YYYY")
                    obs_emp = st.text_input("Observações / Ocorrências:", key="obs_emp_v84")
                    
                    if st.button("📥 Concluir e Registrar Empréstimo", key="btn_concluir_emp_v84"):
                    ...
                    if not df_emprestimos.empty:
                        ...
                        if not df_emp_ano.empty:
                            ...
                            if lista_emp_ativos:
                                emp_selecionado_acao = st.selectbox("Selecione o empréstimo para dar Baixa (Devolução):", ["Selecione..."] + lista_emp_ativos, key="sel_baixa_v84")
            ...
            sub_pbf = st.sidebar.radio("Sub-menu:", ["Importar Dados", "Visualizar Dados", "Imprimir / Relatório", "Atualização em Lote (PBF)"], key="sub_pbf_v84")
            ...
            if sub_pbf == "Importar Dados":
                ...
                periodo_imp = st.selectbox("Selecione o Período de Referência:", periodos_pbf, key="sel_periodo_pbf_imp_v84")
                ...
                if tem_dados_salvos:
                    ...
                    conf_sobrescrever = st.radio("Deseja sobrescrever os dados existentes? Esta operação é irreversível!", ["Escolha...", "Não", "Sim"], index=0, key="radio_sobrescrever_pbf_v84")
                ...
                if not tem_dados_salvos or sobrescrever_autorizado:
                    if not tem_dados_salvos or (tem_dados_salvos and conf_sobrescrever == "Sim"):
                        arquivo_pbf = st.file_uploader(f"Carregar arquivo PDF para o período [{periodo_imp}] ({ano_letivo_escolhido}):", type=["pdf"], key=f"upl_pbf_pdf_v84_{periodo_imp}")
                        
                        if arquivo_pbf is not None:
                            if st.button("📥 Processar e Salvar no Banco PBF", key="btn_salvar_pbf_lote_v84"):
                ...
                periodo_vis = st.selectbox("Selecione o Período de Referência para Visualização:", periodos_pbf, key="sel_periodo_pbf_vis_v84")
                ...
                periodo_imp_ref = st.selectbox("Selecione o Período de Referência para Relatório:", periodos_pbf, key="sel_periodo_pbf_imp_ref_v84")
                ...
                if not df_pbf_rel.empty:
                    ...
                    html_botao_impressao = (
                        ...
                        "'<div>Sistemas iPeC - v.1.5.084</div>' +"
                ...
                periodo_lote_pbf = st.selectbox("Selecione o Período de Referência Base:", periodos_pbf, key="sel_periodo_pbf_lote_rotina_v84")
                
                confirma_lote = st.radio("⚠️ Atenção: Os dados do cadastro desses alunos serão alterados. Deseja continuar?", ["Escolha...", "Não", "Sim"], index=0, key="radio_confirma_lote_pbf_v84")
            ...
            if st.session_state["perfil_usuario"] == "Total":
                sub_sup_adm = st.sidebar.radio("Sub-menu Sup:", ["Cadastrar Novo Usuário", "Logs de Auditoria em Tempo Real"], key="sub_sup_adm_v84")
                if sub_sup_adm == "Cadastrar Novo Usuário":
                    st.markdown("#### 👤 Painel Administrativo: Cadastro de Novo Usuário")
                    with st.form("form_cad_novo_usuario_v84"):
