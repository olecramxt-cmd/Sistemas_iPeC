...
# Programa app.py. Versão do Código: v.1.5.073
# Data de atualização: 26/07/2026 - 19:57
# Descrição das Alterações:
#   - Ajuste final no layout de impressão do relatório PBF: fonte Times New Roman em todo o relatório, redimensionamento da logo, centralização dos títulos institucionais, rodapé em linha única contendo apenas a versão, operador (sem data/hora) e paginação real, e alteração da coluna da tabela de Nº para Id.
# ==============================================================================
...
st.sidebar.markdown("""
    <div class="sidebar-logo-footer">
        Versão: v.1.5.073 de 26/07/2026<br>
    ...
    if ano_letivo_escolhido == "Selecione...":
        ...
        if menu_principal == "📊 Painel de Controle de Conformidade e Indicadores de Alunos":
            ...
            if df_db_ano.empty and sub_conformidade == "Atualização de Dados":
                ...
                if sub_conformidade == "Cadastro de Alunos":
                    ...
                    if df_filtrado.empty:
                        ...
                        df_editavel = st.data_editor(df_filtrado, use_container_width=True, hide_index=True, key="editor_dados_tabela_v73")
                        ...
                        if st.session_state["perfil_usuario"] == "Total":
                            ...
                            with col_bt3:
                                ...
                                aluno_para_excluir = st.selectbox("Selecionar para Exclusão:", lista_excluir_op, key="sel_exc_painel_v73")
                    ...
                    if df_db_ano.empty:
                        ...
                        aluno_selecionado_busca = st.selectbox("Selecione o aluno para alteração individual:", lista_alunos_cadastrados, key="sel_aluno_atualizacao_individual_v73")
                        ...
                        if aluno_selecionado_busca and aluno_selecionado_busca != "Selecione o Aluno...":
                            try:
                                ...
                                if not df_aluno_ind.empty:
                                    ...
                                    with st.form(f"form_atualizacao_individual_v73_{id_alvo_ind}"):
            ...
            if sub_biblioteca == "Catálogo do Acervo":
                ...
                with col_p1:
                    termo_titulo = st.text_input("Filtrar por Título da Obra:", key="f_tit_v73")
                with col_p2:
                    termo_autor = st.text_input("Filtrar por Autor / Organizador:", key="f_aut_v73")
                with col_p3:
                    filtro_cat = st.selectbox("Filtrar por Categoria:", ["Todas", "Didático", "Literário"], key="f_cat_v73")
                ...
                with st.form("form_biblioteca_v73", clear_on_submit=False):
                ...
                if st.session_state.get("acionou_exclusao_form", False):
                    ...
                    confirma_excluir_form = st.radio("Deseja realmente prosseguir com a exclusão deste livro?", ["Não", "Sim"], index=0, key="radio_conf_exc_v73")
                ...
                with st.form("form_config_biblioteca_v73"):
                ...
                sub_aba_emp = st.radio("Gestão de Circulação:", ["Novo Empréstimo", "Consulta de Empréstimos por Aluno", "Empréstimos Ativos / Devoluções / Atrasos", "Reservas de Livros"], horizontal=True, key="sub_aba_emp_v73")
                
                if sub_aba_emp == "Novo Empréstimo":
                    aluno_emp_sel = st.selectbox("Selecione o Leitor (Aluno):", ["Selecione..."] + lista_alunos_op_global, key="sel_leitor_v73")
                    livro_emp_sel = st.selectbox("Selecione o Item do Acervo (Livro):", ["Selecione..."] + lista_livros_op_global, key="sel_livro_v73")
                    data_emp = st.date_input("Data do Empréstimo:", value=hoje_dt, key="dt_emp_v73", format="DD/MM/YYYY")
                    ...
                    data_prev = st.date_input("Devolver até:", value=data_prev_calc, key="dt_prev_v73", format="DD/MM/YYYY")
                    obs_emp = st.text_input("Observações / Ocorrências:", key="obs_emp_v73")
                    
                    if st.button("📥 Concluir e Registrar Empréstimo", key="btn_concluir_emp_v73"):
                    ...
                    if not df_emprestimos.empty:
                        ...
                        if not df_emp_ano.empty:
                            ...
                            if lista_emp_ativos:
                                emp_selecionado_acao = st.selectbox("Selecione o empréstimo para dar Baixa (Devolução):", ["Selecione..."] + lista_emp_ativos, key="sel_baixa_v73")
            ...
            sub_pbf = st.sidebar.radio("Sub-menu:", ["Importar Dados", "Visualizar Dados", "Imprimir / Relatório", "Atualização em Lote (PBF)"], key="sub_pbf_v73")
            ...
            if sub_pbf == "Importar Dados":
                ...
                if tem_dados_salvos:
                    ...
                    conf_sobrescrever = st.radio("Deseja sobrescrever os dados existentes? Esta operação é irreversível!", ["Escolha...", "Não", "Sim"], index=0, key="radio_sobrescrever_pbf")
                ...
                if not tem_dados_salvos or sobrescrever_autorizado:
                    if not tem_dados_salvos or (tem_dados_salvos and conf_sobrescrever == "Sim"):
                        arquivo_pbf = st.file_uploader(f"Carregar arquivo PDF para o período [{periodo_imp}] ({ano_letivo_escolhido}):", type=["pdf"], key=f"upl_pbf_pdf_{periodo_imp}")
                ...
                periodo_vis = st.selectbox("Selecione o Período de Referência para Visualização:", periodos_pbf, key="sel_periodo_pbf_vis")
                ...
                if not df_pbf_rel.empty:
                    ...
                    # Montagem segura da tabela HTML com numeração sequencial (coluna Id.)
                    linhas_html_tabela = ""
                    for idx_t, r_t in df_pbf_rel.reset_index(drop=True).iterrows():
                        ...
                        linhas_html_tabela += f"<tr><td style='text-align: center; width: 45px;'>{num_seq}</td><td>{nome_al}</td><td>{turma_al}</td></tr>"

                    subtitulo_dinamico = f"RELATÓRIO DE ALUNOS INSCRITOS NO PROGRAMA DO BOLSA FAMÍLIA"
                    periodo_ref_texto = f"PERÍODO DE REFERÊNCIA: {periodo_imp_ref}/{ano_letivo_escolhido}"
                    ...
                    html_botao_impressao = (
                        ...
                        "'<style>' +"
                        "'@page { size: A4; margin: 15mm 15mm 18mm 15mm; @top-left { content: \"" + obter_horario_unai().strftime("%d/%m/%Y %H:%M") + "\"; font-family: \"Times New Roman\", Times, serif; font-size: 10pt; } @top-center { content: \"RELATÓRIO PBF\"; font-family: \"Times New Roman\", Times, serif; font-size: 10pt; font-weight: bold; } @bottom-left { content: \"© Sistemas iPeC - v.1.5.073\"; font-family: \"Times New Roman\", Times, serif; font-size: 10pt; } @bottom-center { content: \"Operador: " + operador_atual + "\"; font-family: \"Times New Roman\", Times, serif; font-size: 10pt; } @bottom-right { content: \"p. \" counter(page) \"/\" counter(pages); font-family: \"Times New Roman\", Times, serif; font-size: 10pt; } }' +"
                        "'body { font-family: \"Times New Roman\", Times, serif; color: #000; margin: 0; padding: 0; font-size: 11pt; }' +"
                        "'.header-table { width: 100%; border-collapse: collapse; border-bottom: 2px solid #000; margin-bottom: 12px; padding-bottom: 8px; }' +"
                        "'.logo-cell { width: 65px; vertical-align: middle; padding-right: 15px; }' +"
                        "'.logo-img { width: 55px; height: auto; display: block; }' +"
                        "'.title-cell { vertical-align: middle; text-align: center; }' +"
                        "'.title-cell h2 { margin: 0 0 4px 0; font-size: 14pt; font-family: \"Times New Roman\", Times, serif; text-transform: uppercase; font-weight: bold; }' +"
                        "'.title-cell h3 { margin: 0 0 4px 0; font-size: 11pt; font-family: \"Times New Roman\", Times, serif; font-weight: bold; color: #333; text-transform: uppercase; }' +"
                        "'.title-cell h4 { margin: 0; font-size: 10pt; font-family: \"Times New Roman\", Times, serif; font-weight: bold; color: #444; }' +"
                        "'table.report-table { width: 100%; border-collapse: collapse; margin-top: 10px; margin-bottom: 25px; }' +"
                        "'table.report-table th, table.report-table td { border: 1px solid #999; padding: 5px 8px; text-align: left; font-size: 11pt; font-family: \"Times New Roman\", Times, serif; }' +"
                        "'table.report-table th { background-color: #0e4166; color: white; text-transform: uppercase; font-weight: bold; font-size: 11pt; }' +"
                        "'table.report-table tr:nth-child(even) { background-color: #f2f2f2; }' +"
                        "'p.print-footer, div.print-footer { display: none !important; }' +"
                        "'</style></head><body>' +"
                        "'<table class=\"header-table\"><tr>' +"
                        "'<td class=\"logo-cell\"><img src=\"data:image/jpeg;base64," + logo_base64_rel + "\" class=\"logo-img\" onerror=\"this.style.display=\\'none\\'\"/></td>' +"
                        "'<td class=\"title-cell\"><h2>ESCOLA MUNICIPAL PROFª GLÓRIA MOREIRA</h2><h3>" + subtitulo_dinamico + "</h3><h4>" + periodo_ref_texto + "</h4></td>' +"
                        "'</tr></table>' +"
                        "'<table class=\"report-table\"><thead><tr><th style=\"text-align: center;\">Id.</th><th>ALUNO</th><th>PERÍODO / TURMA</th></tr></thead><tbody>' +"
                ...
                confirma_lote = st.radio("⚠️ Atenção: Os dados do cadastro desses alunos serão alterados. Deseja continuar?", ["Escolha...", "Não", "Sim"], index=0, key="radio_confirma_lote_pbf")
