...
# Programa app.py. Versão do Código: v.1.5.095
# Data de atualização: 02/08/2026 - 08:22
# Descrição das Alterações:
#   - Exclusão da linha superior de data/título no topo da Ficha Individual (v.1.5.095).
#   - Padronização rigorosa do rodapé corporativo (Esquerda, Centro com Operador, Direita com p.xx/xx).
#   - Inclusão do rótulo "📋 Prévia dos Dados da Turma" no sub-menu Relatório do PBF.
#   - Blindagem total de impressão contra URLs e sobreposições do navegador.
# ==============================================================================
...
st.sidebar.markdown("""
    <div class="sidebar-logo-footer">
        Versão: v.1.5.095 de 02/08/2026<br>
        ...
        if menu_principal == "📊 Painel de Controle de Conformidade e Indicadores de Alunos":
            ...
            if df_db_ano.empty and sub_conformidade in ["Atualização de Dados", "Relatórios"]:
                ...
                if sub_conformidade == "Relatórios":
                    ...
                    sub_sub_rel_painel = st.radio("Selecione o Relatório:", ["Ficha Individual"], horizontal=True, key="sub_sub_rel_painel_v95")
                    ...
                    if sub_sub_rel_painel == "Ficha Individual":
                        ...
                        aluno_ficha_sel = st.selectbox("Selecione o Aluno para Emissão da Ficha Individual:", lista_alunos_ficha, key="sel_aluno_ficha_ind_v95")
                        ...
                        if aluno_ficha_sel and aluno_ficha_sel != "Selecione o Aluno...":
                            ...
                            html_botao_impressao_ficha = (
                                ...
                                "'<style>' +"
                                "'@page { size: A4; margin: 20mm; margin-bottom: 25mm; }' +"
                                "'body { font-family: \"Times New Roman\", Times, serif; font-size: 12pt; line-height: 1.5; color: #000; margin: 0; padding: 0; counter-reset: page; }' +"
                                ...
                                "'@media print { .footer-container { position: fixed; bottom: -15mm; } }' +"
                                "'.page-number::before { counter-increment: page; content: \"p.\" counter(page); }' +"
                                ...
                                "'<div style=\"width: 33%; text-align: left;\">© Sistemas iPeC - v.1.5.095</div>' +"
                                "'<div style=\"width: 34%; text-align: center;\">Operador: " + operador_atual + "</div>' +"
                                "'<div style=\"width: 33%; text-align: right;\"><span class=\"page-number\"></span></div>' +"
                                "'</div></body></html>';"
                    ...
                    def atualizar_filtro_aluno():
                        st.session_state.f_aluno = st.session_state.input_f_aluno_v95
                    def atualizar_filtro_mae():
                        st.session_state.f_mae = st.session_state.input_f_mae_v95
                    def atualizar_filtro_turma():
                        st.session_state.f_turma = st.session_state.input_f_turma_v95
                    def atualizar_filtro_turno():
                        st.session_state.f_turno = st.session_state.input_f_turno_v95
                    def atualizar_filtro_status():
                        st.session_state.f_status = st.session_state.input_f_status_v95
                    def atualizar_filtro_pbf():
                        st.session_state.f_pbf = st.session_state.input_f_pbf_v95

                    filtro_cols = st.columns(2)
                    with filtro_cols[0]:
                        st.text_input("Filtrar por Aluno:", value=st.session_state.f_aluno, key="input_f_aluno_v95", on_change=atualizar_filtro_aluno)
                        st.text_input("Filtrar por Mãe:", value=st.session_state.f_mae, key="input_f_mae_v95", on_change=atualizar_filtro_mae)
                        st.text_input("Filtrar por Turma:", value=st.session_state.f_turma, key="input_f_turma_v95", on_change=atualizar_filtro_turma)
                    with filtro_cols[1]:
                        st.text_input("Filtrar por Turno:", value=st.session_state.f_turno, key="input_f_turno_v95", on_change=atualizar_filtro_turno)
                        st.text_input("Filtrar por Status:", value=st.session_state.f_status, key="input_f_status_v95", on_change=atualizar_filtro_status)
                        st.text_input("Filtrar por PBF (Sim/Não):", value=st.session_state.f_pbf, key="input_f_pbf_v95", on_change=atualizar_filtro_pbf)
                    ...
                    if df_filtrado.empty:
                        ...
                        st.dataframe(df_estilizado, use_container_width=True, hide_index=True, key="editor_dados_tabela_v95")
                        ...
                        if st.session_state["perfil_usuario"] == "Total":
                            ...
                            with col_bt3:
                                ...
                                aluno_para_excluir = st.selectbox("Selecionar para Exclusão:", lista_excluir_op, key="sel_exc_painel_v95")
                    ...
                    if df_db_ano.empty:
                        ...
                        aluno_selecionado_busca = st.selectbox("Selecione o aluno para alteração individual:", lista_alunos_cadastrados, key="sel_aluno_atualizacao_individual_v95")
                        ...
                        if aluno_selecionado_busca and aluno_selecionado_busca != "Selecione o Aluno...":
                            try:
                                ...
                                if not df_aluno_ind.empty:
                                    ...
                                    with st.form(f"form_atualizacao_individual_v95_{id_alvo_ind}"):
            ...
            if st.session_state["perfil_usuario"] != "Total":
                ...
                if sub_lote == "Importar Arquivo .TXT":
                    ...
                    arquivos_escolhidos = st.file_uploader("Escolha os arquivos de lote (.TXT)", accept_multiple_files=True, key="upl_lote_txt_v95")
                    
                    if arquivos_escolhidos:
                        st.markdown("---")
                        confirma_importacao = st.radio("Confirma importação em Lote?", ["Escolha...", "Não", "Sim"], index=0, key="radio_confirma_imp_lote_v95")
                    ...
                    try:
                        ...
                        if len(dados_hist_raw) > 1:
                            ...
                            if linhas_validas:
                                ...
                                item_hist_sel = st.selectbox("Selecione a linha do histórico:", lista_op_hist, key="sel_hist_apagar_v95")
                                ...
                                if item_hist_sel != "Selecione o registro para apagar...":
                                    ...
                                    confirma_exc_hist = st.radio("Deseja realmente apagar esta linha do histórico?", ["Não", "Sim"], index=0, key="radio_conf_exc_hist_v95")
            ...
            if sub_miguilim == "Triagem de Acuidade":
                ...
                sub_sub_mig = st.radio("Selecione a Opção de Relatório:", ["Encaminhamentos Clínicos", "Carta de Apresentação", "Formulário de Preenchimento"], horizontal=True, key="sub_sub_mig_v95")
            ...
            if sub_biblioteca == "Catálogo do Acervo":
                ...
                with col_p1:
                    termo_titulo = st.text_input("Filtrar por Título da Obra:", key="f_tit_v95")
                with col_p2:
                    termo_autor = st.text_input("Filtrar por Autor / Organizador:", key="f_aut_v95")
                with col_p3:
                    filtro_cat = st.selectbox("Filtrar por Categoria:", ["Todas", "Didático", "Literário"], key="f_cat_v95")
                ...
                with st.form("form_biblioteca_v95", clear_on_submit=False):
                ...
                if st.session_state.get("acionou_exclusao_form", False):
                    ...
                    confirma_excluir_form = st.radio("Deseja realmente prosseguir com a exclusão deste livro?", ["Não", "Sim"], index=0, key="radio_conf_exc_v95")
                ...
                with st.form("form_config_biblioteca_v95"):
                ...
                sub_aba_emp = st.radio("Gestão de Circulação:", ["Novo Empréstimo", "Consulta de Empréstimos por Aluno", "Empréstimos Ativos / Devoluções / Atrasos", "Reservas de Livros"], horizontal=True, key="sub_aba_emp_v95")
                
                if sub_aba_emp == "Novo Empréstimo":
                    aluno_emp_sel = st.selectbox("Selecione o Leitor (Aluno):", ["Selecione..."] + lista_alunos_op_global, key="sel_leitor_v95")
                    livro_emp_sel = st.selectbox("Selecione o Item do Acervo (Livro):", ["Selecione..."] + lista_livros_op_global, key="sel_livro_v95")
                    data_emp = st.date_input("Data do Empréstimo:", value=hoje_dt, key="dt_emp_v95", format="DD/MM/YYYY")
                    ...
                    data_prev = st.date_input("Devolver até:", value=data_prev_calc, key="dt_prev_v95", format="DD/MM/YYYY")
                    obs_emp = st.text_input("Observações / Ocorrências:", key="obs_emp_v95")
                    
                    if st.button("📥 Concluir e Registrar Empréstimo", key="btn_concluir_emp_v95"):
                    ...
                    if not df_emprestimos.empty:
                        ...
                        if not df_emp_ano.empty:
                            ...
                            if lista_emp_ativos:
                                emp_selecionado_acao = st.selectbox("Selecione o empréstimo para dar Baixa (Devolução):", ["Selecione..."] + lista_emp_ativos, key="sel_baixa_v95")
            ...
            sub_pbf = st.sidebar.radio("Sub-menu:", ["Importar Dados", "Visualizar Dados", "Relatório", "Atualização em Lote (PBF)"], key="sub_pbf_v95")
            ...
            if sub_pbf == "Importar Dados":
                ...
                periodo_imp = st.selectbox("Selecione o Período de Referência:", periodos_pbf, key="sel_periodo_pbf_imp_v95")
                ...
                if tem_dados_salvos:
                    ...
                    conf_sobrescrever = st.radio("Deseja sobrescrever os dados existentes? Esta operação é irreversível!", ["Escolha...", "Não", "Sim"], index=0, key="radio_sobrescrever_pbf_v95")
                ...
                if not tem_dados_salvos or sobrescrever_autorizado:
                    if not tem_dados_salvos or (tem_dados_salvos and conf_sobrescrever == "Sim"):
                        arquivo_pbf = st.file_uploader(f"Carregar arquivo PDF para o período [{periodo_imp}] ({ano_letivo_escolhido}):", type=["pdf"], key=f"upl_pbf_pdf_v95_{periodo_imp}")
                        
                        if arquivo_pbf is not None:
                            if st.button("📥 Processar e Salvar no Banco PBF", key="btn_salvar_pbf_lote_v95"):
                ...
                periodo_vis = st.selectbox("Selecione o Período de Referência para Visualização:", periodos_pbf, key="sel_periodo_pbf_vis_v95")
                ...
                periodo_imp_ref = st.selectbox("Selecione o Período de Referência para Relatório:", periodos_pbf, key="sel_periodo_pbf_imp_ref_v95")
                
                tipo_rel_pbf = st.radio("Selecione o Tipo de Relatório:", ["Por Turma", "Geral"], horizontal=True, key="radio_tipo_rel_pbf_v95")
                
                # Inserção do rótulo solicitado logo abaixo do filtro
                ...
                if not df_pbf_rel.empty:
                    ...
                    if tipo_rel_pbf == "Por Turma":
                        ...
                        turma_escolhida_filtro = st.selectbox("Selecione a Turma / Período para o Relatório:", ["Todas"] + turmas_disponiveis_pbf, key="sel_turma_pbf_filtro_v95")
                ...
                periodo_lote_pbf = st.selectbox("Selecione o Período de Referência Base:", periodos_pbf, key="sel_periodo_pbf_lote_rotina_v95")
                
                confirma_lote = st.radio("⚠️ Atenção: Os dados do cadastro desses alunos serão alterados. Deseja continuar?", ["Escolha...", "Não", "Sim"], index=0, key="radio_confirma_lote_pbf_v95")
            ...
            if st.session_state["perfil_usuario"] == "Total":
                sub_sup_adm = st.sidebar.radio("Sub-menu Sup:", ["Cadastrar Novo Usuário", "Logs de Auditoria em Tempo Real"], key="sub_sup_adm_v95")
                if sub_sup_adm == "Cadastrar Novo Usuário":
                    st.markdown("#### 👤 Painel Administrativo: Cadastro de Novo Usuário")
                    with st.form("form_cad_novo_usuario_v95"):
