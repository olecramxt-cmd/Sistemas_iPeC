...
# Programa app.py. Versão do Código: v.1.5.054
# Data de atualização: 26/07/2026 - 06:27
# Descrição das Alterações:
#   - Correção da extração de PDFs no módulo PBF com a biblioteca pypdf para eliminar caracteres corrompidos.
#   - Implementação do botão de impressão interativo com janela de seleção de impressora adaptada ao padrão iPeC.
# ==============================================================================
...
st.sidebar.markdown("""
    <div class="sidebar-logo-footer">
        Versão: v.1.5.054 de 26/07/2026<br>
    ...
    if ano_letivo_escolhido == "Selecione...":
        ...
        if menu_principal == "📊 Painel de Controle de Conformidade e Indicadores de Alunos":
            ...
            if df_db_ano.empty and sub_conformidade == "Atualização de Dados":
                ...
                if sub_conformidade == "Cadastro de Alunos":
                    ...
                    if df_filtrado.empty:
                        ...
                        df_editavel = st.data_editor(df_filtrado, use_container_width=True, hide_index=True, key="editor_dados_tabela_v54")
                        ...
                        if st.session_state["perfil_usuario"] == "Total":
                            ...
                            with col_bt3:
                                ...
                                aluno_para_excluir = st.selectbox("Selecionar para Exclusão:", lista_excluir_op, key="sel_exc_painel_v54")
                    ...
                    if df_db_ano.empty:
                        ...
                        aluno_selecionado_busca = st.selectbox("Selecione o aluno para alteração individual:", lista_alunos_cadastrados, key="sel_aluno_atualizacao_individual_v54")
                        ...
                        if aluno_selecionado_busca and aluno_selecionado_busca != "Selecione o Aluno...":
                            try:
                                ...
                                if not df_aluno_ind.empty:
                                    ...
                                    with st.form(f"form_atualizacao_individual_v54_{id_alvo_ind}"):
            ...
            if sub_biblioteca == "Catálogo do Acervo":
                ...
                with col_p1:
                    termo_titulo = st.text_input("Filtrar por Título da Obra:", key="f_tit_v54")
                with col_p2:
                    termo_autor = st.text_input("Filtrar por Autor / Organizador:", key="f_aut_v54")
                with col_p3:
                    filtro_cat = st.selectbox("Filtrar por Categoria:", ["Todas", "Didático", "Literário"], key="f_cat_v54")
                ...
                with st.form("form_biblioteca_v54", clear_on_submit=False):
                ...
                if st.session_state.get("acionou_exclusao_form", False):
                    ...
                    confirma_excluir_form = st.radio("Deseja realmente prosseguir com a exclusão deste livro?", ["Não", "Sim"], index=0, key="radio_conf_exc_v54")
                ...
                with st.form("form_config_biblioteca_v54"):
                ...
                sub_aba_emp = st.radio("Gestão de Circulação:", ["Novo Empréstimo", "Consulta de Empréstimos por Aluno", "Empréstimos Ativos / Devoluções / Atrasos", "Reservas de Livros"], horizontal=True, key="sub_aba_emp_v54")
                
                if sub_aba_emp == "Novo Empréstimo":
                    aluno_emp_sel = st.selectbox("Selecione o Leitor (Aluno):", ["Selecione..."] + lista_alunos_op_global, key="sel_leitor_v54")
                    livro_emp_sel = st.selectbox("Selecione o Item do Acervo (Livro):", ["Selecione..."] + lista_livros_op_global, key="sel_livro_v54")
                    data_emp = st.date_input("Data do Empréstimo:", value=hoje_dt, key="dt_emp_v54", format="DD/MM/YYYY")
                    ...
                    data_prev = st.date_input("Devolver até:", value=data_prev_calc, key="dt_prev_v54", format="DD/MM/YYYY")
                    obs_emp = st.text_input("Observações / Ocorrências:", key="obs_emp_v54")
                    
                    if st.button("📥 Concluir e Registrar Empréstimo", key="btn_concluir_emp_v54"):
                    ...
                    if not df_emprestimos.empty:
                        ...
                        if not df_emp_ano.empty:
                            ...
                            if lista_emp_ativos:
                                emp_selecionado_acao = st.selectbox("Selecione o empréstimo para dar Baixa (Devolução):", ["Selecione..."] + lista_emp_ativos, key="sel_baixa_v54")
            ...
            sub_pbf = st.sidebar.radio("Sub-menu:", ["Importar Dados", "Visualizar Dados", "Imprimir / Relatório", "Atualização em Lote (PBF)"], key="sub_pbf_v54")
            ...
            if sub_pbf == "Importar Dados":
                ...
                st.info("Selecione o período de referência abaixo e carregue o arquivo PDF oficial para extração limpa e segura dos nomes dos alunos.")
                
                periodo_imp = st.selectbox("Selecione o Período de Referência:", periodos_pbf, key="sel_periodo_pbf_imp")
                arquivo_pbf = st.file_uploader(f"Carregar arquivo PDF para o período [{periodo_imp}] ({ano_letivo_escolhido}):", type=["pdf", "txt", "csv"], key=f"upl_pbf_{periodo_imp}")
                
                if arquivo_pbf is not None:
                    if st.button("📥 Processar e Salvar no Banco PBF", key="btn_salvar_pbf_lote"):
                        try:
                            linhas_extraidas = []
                            if arquivo_pbf.name.endswith(".pdf"):
                                ...
                                # Processamento inteligente para extrair nomes limpos de relatórios de frequência
                                linhas_brutas = texto_total.split("\n")
                                ignorar_termos = ["Sistema", "PRESENÇA", "Relatório", "DADOS DA ESCOLA", "Nome da Escola", "INEP", "UF/Município", "Ano", "Legenda", "Lançamento", "Sem lançamento", "Alunos S/Freq."]
                                
                                for linha in linhas_brutas:
                                    linha_l = linha.strip()
                                    if len(linha_l) > 4 and not any(ig in linha_l for ig in ignorar_termos) and not "2026(" in linha_l and not "FEV." in linha_l and not "MAR." in linha_l:
                                        # Remove pontuações isoladas de rodapé ou datas
                                        if not re.match(r'^\d{2}/\d{2}/\d{4}', linha_l):
                                            linhas_extraidas.append({"Aluno": linha_l})
                            else:
                                stringio = arquivo_pbf.getvalue().decode("latin1")
                                for linha in stringio.split("\n"):
                                    if len(linha.strip()) > 3:
                                        linhas_extraidas.append({"Aluno": linha.strip()})
                ...
                if not df_pbf_vis.empty:
                    st.success(f"Exibindo {len(df_pbf_vis)} registros para o período {periodo_vis} ({ano_letivo_escolhido}).")
                ...
                if not df_pbf_rel.empty:
                    ...
                    # Botão com janela de seleção de impressora nativa adaptada ao padrão iPeC
                    html_botao_impressao = f"""
                    <script>
                    function abrirJanelaImpressora() {{
                        window.print();
                    }}
                    </script>
                    <button onclick="abrirJanelaImpressora()" style="background-color: #0e4166; color: white; padding: 10px 20px; border: none; border-radius: 5px; font-weight: bold; cursor: pointer; font-size: 16px;">
                        🖨️ Selecionar Impressora e Imprimir Listagem
                    </button>
                    """
                    st.components.v1.html(html_botao_impressao, height=60)
                else:
